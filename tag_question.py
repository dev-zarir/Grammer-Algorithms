...
def get_simple_sent(sent:str):
    sent=get_last_sent(sent).lower().replace('let us', 'we shall not').replace("let's", 'we shall not').replace("'ll",' will').replace("'m", ' am').replace("'s", ' is').replace("n't", " not").replace("'re"," are").replace("'ve"," have").split(' ')
    if sent[0] == 'let':
        if sent[1] != 'us':
            sent[0] = 'you'
            sent[1] = 'will not'
