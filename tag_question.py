...
negative_words= ['nothing', 'nobody', 'none', 'never', 'hardly', 'scarcely', 'seldom', 'rarely', 'few', 'little']
...
def get_simple_sent(sent:str):
    sent=get_last_sent(sent).lower().replace('let us', 'we shall not').replace("let's", 'we shall not').replace("'ll",' will').replace("'m", ' am').replace("it's", 'it is').replace("n't", " not").split(' ')
    if sent[0] == 'let':
        if sent[1] != 'us':
            sent[0] = 'you'
            sent[1] = 'will not'
